...
Every noumen agent has access to a set of built-in tools registered in the `ToolRegistry`. The six core tools (ReadFile, WriteFile, EditFile, Bash, Glob, Grep) are always available. Additional tools are enabled through `Code` options — subagents, tasks, plan mode, worktrees, LSP, web search, and skills each add their own tools when enabled.

All tools delegate file and shell access through `VirtualFs` and `VirtualComputer`, so isolation is controlled entirely by which implementations you provide.
...
const code = new Code({
  ...
  options: {
    ...
    enableLsp: true,          // adds LSP tool (requires lspServers config)
