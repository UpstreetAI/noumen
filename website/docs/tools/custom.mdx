...
noumen's `Tool` interface lets you define custom tools that the agent can call during its runs. Custom tools are passed through `Agent` options and are available alongside the built-in tools.
...
Pass tools in the `Agent` options:
