...
Options:
  -p, --provider <name>    openai | anthropic | gemini | openrouter | bedrock | vertex
...
If no `--provider` is specified, the CLI auto-detects from whichever env var is set.

## Provider auto-detection

When no provider is specified, the CLI checks environment variables in order:
