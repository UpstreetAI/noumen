...
```bash
npx noumen --headless -p anthropic -m claude-sonnet-4-20250514
```
...
This works from any language that can spawn a subprocess. See the [Server API Reference](/docs/server-api#headless-cli-protocol) for the full protocol and examples in Node.js and Python.
...
noumen doctor
  ...
  ✓ Provider (claude-sonnet-4-20250514)  (342ms)
...
The same check is available programmatically via `code.diagnose()` — see [Embedding](/docs/embedding) for details.
