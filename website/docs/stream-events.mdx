...
| Event | Fields | Description |
|-------|--------|-------------|
| `structured_output` | `data`, `schema` | Structured output was produced matching the requested schema |
| `max_turns_reached` | `maxTurns`, `turnCount` | The agent hit the maxTurns limit and stopped |
...
| `maxTurns` | `number` | Maximum number of agent turns before stopping. Emits `max_turns_reached` when hit. |
| `outputFormat` | `OutputFormat` | Request structured JSON output from the model (JSON schema or JSON object mode). |
