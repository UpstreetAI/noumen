...
The two local factories live on the root barrel. Each remote backend lives on its own subpath so its optional peer dependency (e.g. `dockerode`, `e2b`, `ssh2`) only enters the module graph when you opt in — `import { Agent } from "noumen"` stays lean regardless of which backends are installed.
